...
# ---------- fetch_audio_192k (mocked) ----------
def test_fetch_audio_192k_full_flow(monkeypatch, tmp_path):
    ...
    fake_video.get_download_url.return_value = fake_url_info

    with patch("scripts.bilibili_audio.video.Video", return_value=fake_video):
        # mock ffmpeg by patching subprocess.run
        ...
        with patch("subprocess.run", side_effect=fake_ffmpeg):
            ...
            result = fetch_audio_192k("BV1xx411c7mD", 0, out, cookie_path=str(cookie_file))

    assert result == out
    assert out.exists()
    assert out.stat().st_size > 100
    fake_video.get_download_url.assert_called_once_with(page_index=0)

def test_fetch_audio_192k_no_audio_tracks(monkeypatch, tmp_path):
    ...
    with patch("scripts.bilibili_audio.video.Video", return_value=fake_video):
        ...
        with pytest.raises(BilibiliAudioError, match="no audio tracks"):
            fetch_audio_192k("BV1xx411c7mD", 0, tmp_path / "out.mp3",
                             cookie_path=str(cookie_file))

def test_fetch_audio_192k_ffmpeg_failure(monkeypatch, tmp_path):
    fake_url_info = {
    ...
    with patch("scripts.bilibili_audio.video.Video", return_value=fake_video), \
        ...
        with pytest.raises(BilibiliAudioError, match="ffmpeg failed"):
            fetch_audio_192k("BV1xx", 0, tmp_path / "out.mp3",
                             cookie_path=str(cookie_file))
